...
if ( ! class_exists( CustomBlocks::class ) ) :
	...
	class CustomBlocks extends CoreComponent {
				...
				function( $item ) {
					...
					if ( $enable_repeater ) {
						...
						$parent_args = [
							...
							'custom_scripts'        => $this->refine_custom_scripts( $block_custom_scripts, 'JS', $parent_block_name ),
							'external_styles'       => $block_external_styles,
							'custom_styles'         => $this->refine_custom_scripts( $block_custom_styles, 'CSS', $parent_block_name ),
							'hideOnFrontend'        => $hide_on_frontend,
		 ...
		 * @return array
		 */
		private function refine_custom_scripts( $custom_scripts, $type, $block_name ) {
			if ( $custom_scripts && is_array( $custom_scripts ) ) {
				...
				$custom_scripts = array_map(
					function ( $script ) use ( $custom_style_handler, $block_class, $type ) {
						$tokens          = 'CSS' === $type ? [ 'selector' => $block_class ] : [ 'BLOCKSELECTOR' => $block_class ];
